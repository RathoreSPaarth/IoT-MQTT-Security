...
byte aes_key[24] = {  // Adjusted to 192-bit key
    'T', 'h', 'i', 's', 'A', '1', '9', '2', 'C', 'h', 'a',
    'r', 'K', 'e', 'y', 'F', 'o', 'r', 'A', 'E', 'S', '1', '9', '2'
};
...
uint16_t encrypt_to_ciphertext(char* msg, uint16_t msgLen, byte iv[]) {
  Serial.println("Calling AES192 encrypt (string)...");
...
void loop() {
  ...
  if (now - lastMsg > 15000) {
    ...
    Serial.print("Base64 Encrypted message (AES192): ");
    Serial.println(base64_encoded);

    // Publish the base64 encoded encrypted message
    if (client.publish("AES192", base64_encoded, encoded_len)) {
      Serial.println("Message published to AES192 topic successfully.");
    } else {
    ...
    Serial.print("Execution time (AES192 Encryption + Publish): ");
